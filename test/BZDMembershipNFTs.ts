...
describe("BZDMembershipNFTs", () => {
  ...
  // Test 18: Transferring membership NFTs should be allowed when transferability is true
  it("Should allow transferring membership NFTs when transferability is true", async () => {
    const members = [addr1.address];
    const seasonId = 1;

    // Mint NFTs and add members to the season
    await contract.mintAndAddMembersToSeason(members, seasonId);
    await contract.setTransferability(seasonId, true);
    expect(await contract.transferable(seasonId)).to.equal(true);
  ...
  // Test 19: Batch transferring membership NFTs should be allowed when transferability is true
  it("Should allow batch transferring membership NFTs when transferability is true", async () => {
    const members = [addr1.address];
    const seasonId = 1;
    ...
    // Start a new season and mint NFTs and add members to the season
    const newSeason = 3;
    await contract.setCurrentSeason(newSeason);
    expect(await contract.currentSeason()).to.equal(newSeason);
    ...
    // Toggle transferability to allow for NFT transfers
    await contract.setTransferability(seasonId, true);
    expect(await contract.transferable(seasonId)).to.equal(true);
    await contract.setTransferability(newSeason, true);
    expect(await contract.transferable(newSeason)).to.equal(true);
    ...
    expect(await contract.balanceOf(addr1.address, seasonId)).to.equal(0);
    expect(await contract.balanceOf(addr1.address, newSeason)).to.equal(0);
    expect(await contract.balanceOf(addr2.address, seasonId)).to.equal(1);
    expect(await contract.balanceOf(addr2.address, newSeason)).to.equal(1);
  });
});
